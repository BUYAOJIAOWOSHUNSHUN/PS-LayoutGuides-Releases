...
async function repairInstallPermission(targetFolder) {
  ...
  const ps1 = [
    ...
    "icacls \"$target\" /grant \"$env:USERNAME:(OI)(CI)M\" /T",
    "Write-Host ''",
    "Write-Host 'Done! Back in Photoshop: click Check Update -> Download and Install Update again.'",
  ...
  try {
    ...
    if (target && /not permitted|denied|EACCES|EPERM|权限/i.test(text)) {
      try {
        ...
        setUpdateStatus("已打开一键修复脚本：在弹出的授权窗口点「是」，等它显示 Done 后，再点一次「检查更新 → 下载并安装更新」即可。");
        return;
      } catch (repairError) {
